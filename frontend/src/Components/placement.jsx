const PlacementSection = () => {
  const topRecruiters = ["Google", "Microsoft", "Amazon", "Infosys", "TCS", "Wipro"]
    ...
    <section className="bg-gray-900 py-20 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16">
          <h3 className="text-white text-2xl font-bold mb-8">Top Recruiters</h3>
          <div className="flex flex-wrap justify-center items-center gap-8">
            {topRecruiters.map((company, index) => (
              <div key={index} className="text-gray-300 text-xl font-semibold hover:text-white transition-colors">
                {company}
              </div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {studentTestimonials.map((student, index) => (
            <div key={index} className="bg-gray-800 rounded-lg p-6">
              ...
              <p className="text-gray-300 text-sm mb-4 italic">"{student.testimonial}"</p>
              <div className="flex justify-between items-center">
                <span className="text-red-400 font-semibold">{student.package}</span>
